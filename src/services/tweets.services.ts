...
class TweetsService {
  async checkAndCreateHashtags(hashtags: string[]) {
    const hashtagDocuments = (await Promise.all(
      hashtags.map((hashtag) => {
        // tìm hashtag trong db, nếu có thì lấyl, k thì tạo mới
  ...
  async getTweetChildren({
    ...
    const tweets = await databaseService.tweets
      ...
      .toArray()
    // Theo Quyen nghĩ thì chỉ tăng view khi get comment page 1
    // Cập nhật view khi getChildren
  ...
  async getNewFeeds({ user_id, limit, page }: { limit: number; page: number; user_id: string }) {
