      ...
      parent_id: {
        custom: {
          options: (value, { req }) => {
            ...
            // Nếu 'type' là retweet, content, quotetweet thì 'parent_id' phải là 'tweet_id' của tweet cha
            if (
      ...
      content: {
        ...
        custom: {
          options: (value, { req }) => {
            ...
            // Nếu type là retweet thì content phải là string ''
...
// Muốn sử dụng asycn await trong handle express thì phải có try catch
// Nếu k dùng try catch phải dùng wrapRequest
export const audienceValidator = wrapRequestHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (tweet.audience === TweetAudience.TwitterCircle) {
    ...
    // Kiểm tra account author còn tồn tại không
    ...
    // Kiểm tra người xem tweet này có trong Twitter circle của author hay không
      ...
      limit: {
        ...
        custom: {
          options: (value, { req }) => {
            ...
            if (num > 100 || num < 1) {
              throw new Error(TWEETS_MESSAGES.LIMIT_INVALID)
            }
            return true
          }
        }
      ...
      page: {
        ...
        custom: {
          options: (value, { req }) => {
            const num = Number(value)
            if (num < 1) {
              throw new Error(TWEETS_MESSAGES.PAGE_INVALID)
            }
            return true
          }
        }
